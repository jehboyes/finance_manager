...
@click.pass_obj
def copy(config, tosetcat, toacadyear, fromsetcat, fromacadyear, omit, close):
    """
    ...
    for model in Base._decl_class_registry.values():
        if hasattr(model, '__tablename__'):
            ...
            if model.__tablename__[:5] == 'input' \
                ...
                # check contains set_id (some don't)
                set_id_col = sum(
    ...
    with DB(config=config) as db:
        ...
        with click.progressbar(set_tables, label="Iterating through category tables...") as bar:
            for table in bar:
                records = []
                table_query = s.query(table).filter(and_(f_set.acad_year == fromacadyear,
                                                         f_set.set_cat_id == fromsetcat))
