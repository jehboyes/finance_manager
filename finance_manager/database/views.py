...
staff_month_ni = ", \n".join(
    [f"dbo.udfNI(mt.p{n}, ni.p{n}, ni.rate)*m.p{n}/mt.p{n} as ni_p{n}" for n in periods()])
staff_month_pension = ", \n".join(
    [f"m.p{n}*ISNULL(pension.p{n},0) as pension_p{n}" for n in periods()])
staff_travel_months = 9
staff_travel_allowance = ", \n ".join(
    [f"s.travel_scheme/{staff_travel_months} as travel_p{n}" for n in range(3+staff_travel_months)])
...
SELECT staff_line_id,
   CASE s.post_type_id
      WHEN 'FRAC' THEN
            dbo.udfFracFTE((fs.curriculum_hours-taught.hours) * \
                           s.indicative_fte / frac_fte.denom, con.work_hours, con.hol_hours)
      ELSE s.indicative_FTE END as FTE
FROM input_pay_staff s
INNER JOIN f_set fs ON fs.set_id = s.set_id
INNER JOIN (SELECT set_id, SUM(s.teaching_hours) as hours FROM input_pay_staff s WHERE s.post_type_id <> 'FRAC' GROUP BY set_id)
	as taught ON taught.set_id = s.set_id
INNER JOIN (SELECT set_id, SUM(s.indicative_FTE) as denom FROM input_pay_staff s WHERE s.post_type_id = 'FRAC' GROUP BY set_id)
...
SELECT s.staff_line_id, s.set_id, f_set.acad_year, f_set.set_cat_id, s.staff_id,
{staff_month_sal}
FROM input_pay_staff s
...
INNER JOIN v_calc_staff_monthly_all a ON a.staff_line_id = s.staff_line_id
""")
         ]
