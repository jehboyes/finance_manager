...
from finance_manager.database.views import account_description, generate_p_string, sql_bound
...
i_periods = generate_p_string("i.p{p} as p{p}", ",")
i_periods_summed = generate_p_string("i.p{p}", "+")
# estimating national_insurance by period - take off an estimate of hourly threshold, multiply by rate
ni_periods = ",\n".join(
    ["("+sql_bound("MAX", f"{rate_calculation}-ni.p{n}/37", "0")+f")*i.p{n}*ni.rate*t.apply_ni as ni_p{n}" for n in periods()])
...
# up to grade 7.
pension_periods = ",\n".join(
    ["("+sql_bound("MIN", "1", f"({rate_calculation}-{spine(4)})/({spine(33)}-{spine(4)})") +
